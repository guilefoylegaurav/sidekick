// Markdown formatting and code block utilities wrapped in a class
...
export class MarkdownRenderer {
  ...
  attachCopyListeners(element) {
    element.querySelectorAll('.copy-btn').forEach((btn) => {
      btn.addEventListener('click', (event) => this.copyCode(event));
    });
  }
  ...
  basicMarkdownFormat(message) {
    ...
    html = html.replace(/```(\w+)?\n([\s\S]*?)```/g, (match, language, code) => {
      return this.formatCodeBlock(language, code);
    });

    // Handle inline code
    html = html.replace(
      /`([^`]+)`/g,
      '<code style="background: #161b22; padding: 2px 4px; border-radius: 3px; font-family: \'JetBrains Mono\', monospace;">$1</code>',
    );
   ...
   * @returns {string} Formatted HTML string
   */
  formatMessageWithCode(message, containerElement = null) {
    let html;

    // Check if marked is available
    if (typeof marked === 'undefined') {
      console.warn('marked.js not loaded, using basic formatting');
      html = this.basicMarkdownFormat(message);
    } else {
    ...
    if (containerElement) {
      ...
      this.attachCopyListeners(containerElement);
    }
